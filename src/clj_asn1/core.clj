...
;; TODO: Make a mapping of class-tags to schema types for their values
...
(schema/defn fill-buffer :- ByteBuffer
  ...
  (cond
    ...
    (byte-array? input)
    (do
      (println "Found input size of " (count input))
...
(defn decode-type-id
  ...
  (let [class (id->class id)
    ...
    (merge {:constructed? (id->constructed? id)
            :class         class}
           (if (= :universal class)
             {:class-tag (get asn1-class-tags tag)}
             {:tag tag}))))
...
(schema/defn universal-type :- Asn1Universal
  [class-tag :- (apply schema/enum (vals asn1-class-tags))
   value]
  {:constructed? false
   :class        :universal
